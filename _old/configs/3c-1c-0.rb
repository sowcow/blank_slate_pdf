$config_loaded = BlankSlatePDF.new('3c-1c-0') do
  configure $setup

  description <<-END
    Two levels of lists and a level of just blank pages.
    Root level list has three columns.
    ---
  ...
  page do
    color ?0*6 do # color #000000
      draw_line grid_x / 3, 0, grid_x / 3, grid_y
      draw_line grid_x * 2 / 3, 0, grid_x * 2 / 3, grid_y
    end
    draw_dots except: [{ x: 0 }, { x: 12 }, { y: 0 }, { y: 16 }, { x: grid_x / 3 }, { x: grid_x * 2 / 3 }]

    [0, grid_x / 3, grid_x * 2 / 3].each { |x|
